...
bool ImGui_ImplSDL3_ProcessEvent(const SDL_Event* event) {
  ...
  switch (event->type) {
    ...
    case SDL_EVENT_FINGER_DOWN: {
      ...
      ImVec2 mouse_pos((float)event->tfinger.x * w, (float)event->tfinger.y * h);
    ...
    case SDL_EVENT_KEY_UP: {
      ...
      io.SetKeyEventNativeData(
        ...
        event->key.keysym.scancode); // To support legacy indexing (<1.87 user code).
...
#define MAP_ANALOG(KEY_NO, AXIS_NO, V0, V1)                                           \
  {                                                                                   \
    float vn = (float)(SDL_GetGamepadAxis(gamepad, AXIS_NO) - V0) / (float)(V1 - V0); \
    vn = IM_SATURATE(vn);                                                             \
